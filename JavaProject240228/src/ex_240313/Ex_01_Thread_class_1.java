	...
	public void run() {
		...
		while (true) {
			try {
				...
				System.out.println("현재 날자: " + currentDate + "현재 시간: " + currentTime);
				sleep(1000);
...
public class Ex_01_Thread_class_1 {
	...
	public static void main(String[] args) {
		...
		// JVM -> 응용프로그램 안에서 (스레드1,스레드2,스레드3,....) 구성이 된다.
		// 실무에서, 실제 웹, 모바일이라든지 작업시,
		// 해당 프레임워크에 내재된 기능 또는 외부 API(기능)에 탑재가 거의 되어있음.
		// 그래서, 처음 주로, 이용하는 편으로, API가 아니라, 우리가 서비스 기능을 무언자 만들 때,
		// 그 때 이용이 될것임.
		// 정의, 사용.
		...
		// 순서2, 스레드를 상속받은 클래스의 인스턴스에서, start 메서드 호출하기.
		tt.start();
