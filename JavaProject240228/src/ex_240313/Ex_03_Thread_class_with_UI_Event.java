...
// 이번에는, 실행하는 클래스에서, 한번에, Runnable 인터페이스도 구현하고.
// JFrame 라고 해서 UI도 같이 상속받기.
public class Ex_03_Thread_class_with_UI_Event extends JFrame implements Runnable {
	...
	Ex_03_Thread_class_with_UI_Event() {
		...
		// MouseAdapter 클래스는, 마우스 클릭, 드래그, 더블 클릭, 클릭해제(마우스 누르고 뗀 순간)
		// 이러한 메서드를 가지고 있는 추상 클래스
		// 그중에서 클릭시 뭔가를 수행하기 위해서, mousePressed 메서드 수행.
		// MouseEvent -> 클릭을 의미함.
		) {
			public void mousePressed(MouseEvent e) {
				// 클릭을 했을 때, 스레드가 살아잇다면, interrupt 방해하는 메서드를 호출하고,
