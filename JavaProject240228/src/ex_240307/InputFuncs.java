...
public class InputFuncs {
	...
	public static Sports enrollSports(Sports sports) {
		...
		System.out.println("스포츠 종목(문자열) 명 입력해주세요>> ");
		String sportsName = scanner.next();
		
		System.out.println("인원수(정수) 입력해주세요>> ");
		int sportsMemberCount = scanner.nextInt();
		
		System.out.println("플레이 시간(문자열) 입력해주세요>> ");
		String sportsPlayTime = scanner.next();
		
		System.out.println("운동 장소(문자열) 입력해주세요>> ");
	    ...
	    System.out.println("대표 사용장비 3명만 입력해주세요.(공백을 기준으로 나란히 작성해주세요.)>> ");
...
//				String seasonInfo, String inOrOut, String[] items, String[] members)
		sports = new SubSports(sportsName, sportsMemberCount, sportsPlayTime, sportsPlayPlace,
				sportsStartSeasonpick, sportsInOrOut, items, members);
