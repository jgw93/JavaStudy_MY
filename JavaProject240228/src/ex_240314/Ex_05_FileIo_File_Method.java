...
public class Ex_05_FileIo_File_Method {
	...
	// 활용은 예) 프로필 이미지등, 처리할 떄, File 클래스 이용해서, 해당 폴더 및 파일등을 임시로 생성등활용.
	...
	public static void listDirectory(File dir) {
		...
		// listFiles(), 본인 폴더 및 하위 폴더 등의 정보를 File 타입 배열로 리턴.
		...
		for(int i=0; i<subFiles.length; i++) {
			...
			System.out.println("파일명 file.getName(): " + file.getName());
			System.out.println("파일크기 file.length(): " + file.length());
			// format 출력 형태로 하기
	...
	public static void main(String[] args) {
		...
		// 오류 코드 읽는 요령
		// 현재, 자바라서 파일이 몇개 없지만,
		// 나중에, 프레임워크에서, 서버단에서 오류가 발생을 하면,
		// 내가 사용한 파일외에, 시스템에서 제공하는 기본 파일등이 있음.
		// 그래서, 결론, 오류코드가 보이면, 먼저) 내가 작업한 파일이 있는지 여부를 확인.
		System.out.println("=============================");
		// 파일인지, 디렉터리 인지 유무 확인, 디렉터리 생성도 확인.
		if (f1.isFile()) {
			System.out.println("f1은 파일입니다.");
		}else {
			System.out.println("f1은 폴더입니다.");
		}
