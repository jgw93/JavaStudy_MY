...
public class Ex_06_Class_1_callByValue {
	...
	public static void main(String[] args) {
		// 메서드 안에서의 변수는 지역변수라서, 효력범위를, 수행후 소명 부분 재확인.
		...
		// call by reference, 참조형 변수, (얕은 복사 생각하면됨) 바로가기.
		
		Circle c1 = new Circle(5);
		// c1 이라는 인스턴스의 멤버에서, radius 라는 값은 : 5 로 할당이 됨.
		// c1 이 바라보는 메모리 위치 주소값 : 예를 들어 0x100이라 한다면
		System.out.println("c1 의 c1.radius 값 조회 : " + c1.getRadius());
		
		// 메서드에, 인자 값으로 레퍼런스 타입을 전달을 함.
		increase2(c1);
		
		System.out.println("increase2 메서드 호출 후 값 확인 : " + c1.getRadius());
		
		// call by ref , 배열 예제, 배열 참조형(인스턴스)
		int[]a = {1,2,3,4,5};
		increase3(a);
		for(int num : a) {
			System.out.println("배열 a의 요소 출력하기, index: " + num);
		}
