...
public class Ex_02_static_1 {
	...
	public static void main(String[] args) {
		A a1 = new A(100,"a1"); // 각각 가리키는 레퍼런스 참조값(메모리 위치 주소값)
		a1.showNumber();
		A a2 = new A(200,"a2");
		a2.showNumber();
		System.out.println("=========================");
		...
		// 결론,
		// 해당 클래스에서 statin 으로 전역으로 선언하면,
		// 다른 인스턴스에서, 해당 전역 변수에 접근시 해당 데이터가 공유됨.
		// a1 에서 전역변수를 변경하고,
		// 다른 인스턴스인 a2 에서 접근을 하니, 전역변수가 변경됨을 확인.
		
		// 만약, 1) 인스턴스 변수로, 해당 클래스 변수에 접근이 가능
		// 2) 반대로 불가능. 클래스 변수로는 인스턴스 변수에 접근이 불가.
		// 인스턴스는 각각의 고유한 독립값.
		// 클래스변수는, 해당 클래스가 로더에 의해서, 로드가 되면, 그때 존재하는 변수(전역변수)
		int a12 = a1.commonNumber; // 1) 가능
		//int a23 = A1.a1.number; // 2) 불가능.
	}
