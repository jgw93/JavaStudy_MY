...
class B extends A2 {
	public B(int x) {
		super(x); // 생략이 되어 있고, 묵시적
		// super(): 약속이라서, 항상 자식의 생성자 호출전에 반드시 부모의 생성자가 호출되는것.
		System.out.println("생성자 B 입니다.");
	}
	// 기본 생성자 없어서, 아래의 클래스에서 오류가 남.
...
class C extends B {
	...
	public class Ex_03_inharitance_1 {
		...
		public static void main(String[] args) {
			...
			// 예시. 27번 라인 앞에 지정한다.
			// 2) 디버그 실행(모드), f11 -> Perspective, 오른쪽 상단에 벌레모양 아이콘으로 변경.
			// step into, 메서드, 생성자 호출시
			// 안으로 들어가기. f5
			// step over, 단순 넘기기(skip) f6
			// 변수등을 넘어갈 때, 또는 메서드를 실행만 하고 넘어갈 때.

			// instacne of 연산자 예제
			// 클래스 구조 : C -> B -> A2
			// 큰집 = 작은집, 다형성, 업캐스팅.
			// 문제접, test1, test2, test3의 원래의 객체를 알기가 어렵다.
			A2 test1 = new A2(0);
			A2 test2 = new B(0);
			A2 test3 = new C(0);
			...
			// 이름이 똑같아서, 시스템 입장에서는, 이 기능이, 부모것인지, 자식것인지 구분이 어려운데.
			// 결론, 자식의 재정의한 메소드를 사용한다.
