...
public class Ex_01_Socket_singlechat_server {
	...
	// 예) win + r -> cmd -> ipconfig -> 아이피를 알려주기. 추가로 포트 : 임의로 9999
	// 파일명 서버, 실행
	
	// 클라이언트 역할 : 서버의 아이피 주소와 포트를 이용해서, 변경.
	// socket = new Socket("localhost",9999); 수정이 필요
	// socket = new Socket("서버 아이피",9999);
	// 파일명 클라이언트, 실행
	...
	public static void main(String[] args) throws IOException {
		...
		// 순서1, 선언 부분
		// 재료 1) inBufferedReader 2) BufferedWriter 3) ServerSocket 4) Socket
		// 5) Scanner
		...
		// 소켓을 이용해서, 통신 하기 위한 도구
		Socket socket = null;
		
		// UI 대신, 스캐너 이용해서 콘솔로 작업.
		Scanner scanner = new Scanner(System.in);
		
		// 파일의 입출력 또는 통신 연결등은 반드시 예외 처리가 필요함.
		try {
			// 순서2 구현 부분,
			// 1) 연결 파트 2) 데이터 전송, 받기
			...
			// 클라이언트와 서버가 socket 이라는 인스턴스를 이용해서, 통신할 예정.
